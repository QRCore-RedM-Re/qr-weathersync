...
RegisterNetEvent('qr-weathersync:server:setWeather', function(weather)
    ...
    if isAllowedToChange(src) then
        ...
        if src > 0 then
            if (success) then TriggerClientEvent('qrcore:Notify', src, 9, Lang:t('weather.updated'), 5000, 0, 'hud_textures', 'check', 'COLOR_WHITE')
            else TriggerClientEvent('qrcore:Notify', src, 9, Lang:t('weather.invalid'), 5000, 0, 'mp_lobby_textures', 'cross', 'COLOR_WHITE')
            end
        end
    end
end)
...
RegisterNetEvent('qr-weathersync:server:setTime', function(hour, minute)
    ...
    if isAllowedToChange(src) then
        ...
        if src > 0 then
            if (success) then TriggerClientEvent('qrcore:Notify', src, 9, Lang:t('time.change', {value = hour, value2 = minute or "00"}), 5000, 0, 'hud_textures', 'check', 'COLOR_WHITE')
            else TriggerClientEvent('qrcore:Notify', src, 9, Lang:t('time.invalid'), 5000, 0, 'mp_lobby_textures', 'cross', 'COLOR_WHITE')
            end
        end
    end
end)
...
RegisterNetEvent('qr-weathersync:server:toggleBlackout', function(state)
    ...
    if isAllowedToChange(src) then
        ...
        if src > 0 then
            if (newstate) then TriggerClientEvent('qrcore:Notify', src, 9, Lang:t('blackout.enabled'), 5000, 0, 'hud_textures', 'check', 'COLOR_WHITE')
            else TriggerClientEvent('qrcore:Notify', src, 9, Lang:t('blackout.disabled'), 5000, 0, 'mp_lobby_textures', 'cross', 'COLOR_WHITE')
            end
        end
    end
end)
...
RegisterNetEvent('qr-weathersync:server:toggleFreezeTime', function(state)
    ...
    if isAllowedToChange(src) then
        ...
        if src > 0 then
            if (newstate) then TriggerClientEvent('qrcore:Notify', src, 9, Lang:t('time.now_frozen'), 5000, 0, 'hud_textures', 'check', 'COLOR_WHITE')
            else TriggerClientEvent('qrcore:Notify', src, 9, Lang:t('time.now_unfrozen'), 5000, 0, 'mp_lobby_textures', 'cross', 'COLOR_WHITE')
            end
        end
    end
end)
...
RegisterNetEvent('qr-weathersync:server:toggleDynamicWeather', function(state)
    ...
    if isAllowedToChange(src) then
        ...
        if src > 0 then
            if (newstate) then TriggerClientEvent('qrcore:Notify', src, 9, Lang:t('weather.now_unfrozen'), 5000, 0, 'hud_textures', 'check', 'COLOR_WHITE')
            else TriggerClientEvent('qrcore:Notify', src, 9, Lang:t('weather.now_frozen'), 5000, 0, 'mp_lobby_textures', 'cross', 'COLOR_WHITE')
            end
        end
    end
end)
...
RegisterCommand('freezetime', function(source, args, rawCommand)
    if isAllowedToChange(source) then
        ...
        if source > 0 then
            if (newstate) then return TriggerClientEvent('qrcore:Notify', source, 9, Lang:t('time.frozenc'), 5000, 0, 'hud_textures', 'check', 'COLOR_WHITE') end
            return TriggerClientEvent('qrcore:Notify', source, 9, Lang:t('time.unfrozenc'), 5000, 0, 'hud_textures', 'check', 'COLOR_WHITE')
        end
        if (newstate) then return print(Lang:t('time.now_frozen')) end
        return print(Lang:t('time.now_unfrozen'))
    end
    TriggerClientEvent('qrcore:Notify', source, 9, Lang:t('error.not_allowed'), 5000, 0, 'mp_lobby_textures', 'cross', 'COLOR_WHITE')
end)
...
RegisterCommand('freezeweather', function(source, args, rawCommand)
    if isAllowedToChange(source) then
        ...
        if source > 0 then
            if (newstate) then return TriggerClientEvent('qrcore:Notify', source, 9, Lang:t('dynamic_weather.enabled'), 5000, 0, 'hud_textures', 'check', 'COLOR_WHITE') end
            return TriggerClientEvent('qrcore:Notify', source, 9, Lang:t('dynamic_weather.disabled'), 5000, 0, 'mp_lobby_textures', 'cross', 'COLOR_WHITE')
        end
...
RegisterCommand('weather', function(source, args, rawCommand)
    if isAllowedToChange(source) then
        if args[1] == nil then
            if source > 0 then return TriggerClientEvent('qrcore:Notify', source, 9, Lang:t('weather.invalid_syntaxc'), 5000, 0, 'mp_lobby_textures', 'cross', 'COLOR_WHITE') end
            return print(Lang:t('weather.invalid_syntax'))
        end
        local success = setWeather(args[1])
        if source > 0 then
            if (success) then return TriggerClientEvent('qrcore:Notify', source, 9, Lang:t('weather.willchangeto', {value = string.lower(args[1])}), 5000, 0, 'hud_textures', 'check', 'COLOR_WHITE') end
            return TriggerClientEvent('qrcore:Notify', source, 9, Lang:t('weather.invalidc'), 5000, 0, 'mp_lobby_textures', 'cross', 'COLOR_WHITE')
        end
        if (success) then return print(Lang:t('weather.updated')) end
        return print(Lang:t('weather.invalid'))
    else
        TriggerClientEvent('qrcore:Notify', Lang:t('error.not_access'), 5000, 0, 'mp_lobby_textures', 'cross', 'COLOR_WHITE')
    end
end)
...
RegisterCommand('blackout', function(source, args, rawCommand)
    ...
    if isAllowedToChange(src) then
        ...
        if src > 0 then
            if (newstate) then return TriggerClientEvent('qrcore:Notify', src, 9, Lang:t('blackout.enabledc'), 5000, 0, 'hud_textures', 'check', 'COLOR_WHITE') end
            return TriggerClientEvent('qrcore:Notify', src, 9, Lang:t('blackout.disabledc'), 5000, 0, 'mp_lobby_textures', 'cross', 'COLOR_WHITE')
        end
        if (newstate) then return print(Lang:t('blackout.enabled')) end
        return print(Lang:t('blackout.disabled'))
    end
    TriggerClientEvent('qrcore:Notify', src, 9, Lang:t('error.not_allowed'), 5000, 0, 'mp_lobby_textures', 'cross', 'COLOR_WHITE')
end)

RegisterCommand('morning', function(source, args, rawCommand)
    if isAllowedToChange(source) then
        setTime(9, 0)
        if source > 0 then return TriggerClientEvent('qrcore:Notify', source, 9, Lang:t('time.morning'), 5000, 0, 'hud_textures', 'check', 'COLOR_WHITE') end
    else
        TriggerClientEvent('qrcore:Notify', source, 9, Lang:t('error.not_allowed'), 5000, 0, 'mp_lobby_textures', 'cross', 'COLOR_WHITE')
    end
end)
...
RegisterCommand('noon', function(source, args, rawCommand)
    if isAllowedToChange(source) then
        setTime(12, 0)
        if source > 0 then return TriggerClientEvent('qrcore:Notify', source, 9, Lang:t('time.noon'), 5000, 0, 'hud_textures', 'check', 'COLOR_WHITE') end
    else
        TriggerClientEvent('qrcore:Notify', source, 9, Lang:t('error.not_allowed'), 5000, 0, 'mp_lobby_textures', 'cross', 'COLOR_WHITE')
    end
end)
...
RegisterCommand('evening', function(source, args, rawCommand)
    if isAllowedToChange(source) then
        setTime(18, 0)
        if source > 0 then return TriggerClientEvent('qrcore:Notify', source, 9, Lang:t('time.evening'), 5000, 0, 'hud_textures', 'check', 'COLOR_WHITE') end
    else
        TriggerClientEvent('qrcore:Notify', source, 9, Lang:t('error.not_allowed'), 5000, 0, 'mp_lobby_textures', 'cross', 'COLOR_WHITE')
    end
end)
...
RegisterCommand('night', function(source, args, rawCommand)
    if isAllowedToChange(source) then
        setTime(23, 0)
        if source > 0 then return TriggerClientEvent('qrcore:Notify', source, 9, Lang:t('time.night'), 5000, 0, 'hud_textures', 'check', 'COLOR_WHITE') end
    else
        TriggerClientEvent('qrcore:Notify', source, 9, Lang:t('error.not_allowed'), 5000, 0, 'mp_lobby_textures', 'cross', 'COLOR_WHITE')
    end
end)
...
RegisterCommand('time', function(source, args, rawCommand)
    if isAllowedToChange(source) then
        if args[1] == nil then
            if source > 0 then return TriggerClientEvent('qrcore:Notify', source, 9, Lang:t('time.invalidc'), 5000, 0, 'mp_lobby_textures', 'cross', 'COLOR_WHITE') end
            return print(Lang:t('time.invalid'))
        end
        local success = setTime(args[1], args[2])
        if source > 0 then
            if (success) then return TriggerClientEvent('qrcore:Notify', source, 9, Lang:t('time.changec', {value = args[1]..':'..(args[2] or "00")}), 5000, 0, 'hud_textures', 'check', 'COLOR_WHITE') end
            return TriggerClientEvent('qrcore:Notify', source, 9, Lang:t('time.invalidc'), 5000, 0, 'mp_lobby_textures', 'cross', 'COLOR_WHITE')
        end
        if (success) then return print(Lang:t('time.change', {value = args[1], value2 = args[2] or "00"})) end
        return print(Lang:t('time.invalid'))
    else
        TriggerClientEvent('qrcore:Notify', source, 9, Lang:t('time.access'), 5000, 0, 'mp_lobby_textures', 'cross', 'COLOR_WHITE')
    end
end)
